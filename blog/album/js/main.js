$(document).ready(function () {
  ...
  var $image = $("<img>");
  var $video = $(
    '<div class="video"><iframe style="width:100%;height:100%;margin:auto;" frameborder="0" allowfullscreen></iframe></div>'
  );
  ...
  var $prevPhoto = $(
    '<div id="prevPhoto"><i class="fa fa-angle-left"></i></div>'
  );
  var $nextPhoto = $(
    '<div id="nextPhoto"><i class="fa fa-angle-right"></i></div>'
  );
  ...
  function fadeImgs() {
    if (currentLocation < imageCache.length) {
      ...
      $image.fadeOut(imgAnimDuration, function () {
        if (
          currentLocation >= 12 ||
          $galleryImgs.attr("data-type") === "video"
        ) {
          $video
            .find("iframe")
            .attr("src", imageCache[currentLocation].itemURL);
  ...
  $galleryImgs.on("click", function (event) {
    ...
    $image.attr("src", imageSelected).css({
      width: "100%",
      height: "100%",
      "object-fit": "contain",
    });
    ...
    $overlay.show().animate({ opacity: "1" }, imgAnimDuration);
  ...
  $search.on("keyup", function () {
    ...
    $gallery.children("a").each(function (index, item) {
      ...
      if (
        $(item).filter("[data-caption *= " + search + "]").length > 0 ||
        search.length < 1
      ) {
